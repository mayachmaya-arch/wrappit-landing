...
export default function Marquee() {
  return (
    // A single fixed-height rectangle, rotated as one piece — not a
    // padding-driven box clipped asymmetrically by a separate wrapper (the
    // previous approach: a flush-top/generous-bottom clip cut the rotated
    // shape unevenly across its width, which is exactly what read as a
    // "trapezoid" with inconsistent thickness rather than a true rotated
    // rectangle). Rotating the whole box — background and content
    // together — keeps its own top/bottom edges straight, parallel, and a
    // constant distance apart everywhere along its length; nothing clips
    // into that shape from outside it.
    //
    // position:absolute (anchored to hero-viewport, which is
    // position:relative) means this no longer participates in
    ...
    // writing direction (unlike a plain `left` offset, which under RTL is
    // relative to this element's own inline-start edge, not x:0 — that
    // mismatch caused a real gap on one side in an earlier version of this
    // component). w-[110vw] bleeds ~5vw past each edge so the rotation's
    // corner recession never exposes a gap at the screen's left/right
    // edges; hero-viewport's overflow-x-clip trims that bleed at the
    // viewport boundary.
    <div
